...
def play():
    ...
    # c = "ђقາܕ௨ඝඍݹशقາח௧ɸໃکۑඥඍݸкටऐݷяටณ๐௧ɴໃz௨৩ຝ๐ஹௐາܕ௧নາڽணѲຯԣஒඥඍڧறඡඦݰݣഡඍÐԤɴසॾγൺ۵Ӿਧಘ"
    c = input('Enter the code: ')
    # イベントの保管リスト
    events_list = decode(c)
    ...
    while True:   # ゲームのメインループ
        ...
        if num_frame == len(events_list):
            pygame.quit()
            sys.exit()
        event = events_list[num_frame - 1]
        if event == 'R':  # 右が押された場合...
        ...
        if event == 'U':
            ...
            if not next_mino.collision(field):
                mino = next_mino
                moved_flag = True
        if event == 'D':  # 下に移動(落下)は盤面的に可能なところまで落ちる
        ...
        clock.tick(50)  # フレームレートを3 (秒間3回更新するぐらいの頻度に設定)
